...
<ul>
				<?php if (is_array($model->containers)) foreach($model->containers as $foreignobj) { 
			...
			?></ul><p><?php echo CHtml::link(
				Yii::t('app','Create'),
				array('/container/create', 'Container' => array('virtualmin_host_id'=>$model->{$model->tableSchema->primaryKey}))
					);  ?></p>